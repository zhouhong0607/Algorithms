...
public class RedBlackBST<Key extends Comparable<Key>, Value>
{
	 ...
	 */
	public Iterable<Key> keySet(Key lo, Key hi)
	{
		if (lo == null)
		...
		keySet(root, queue, lo, hi);
		return queue;
	}

	// add the keys between lo and hi in the subtree rooted at x
	// to the queue
	private void keySet(Node x, Stack<Key> queue, Key lo, Key hi)
	{
		if (x == null)
			return;
		...
		if (cmplo < 0)
			keySet(x.left, queue, lo, hi);
		if (cmplo <= 0 && cmphi >= 0)
			queue.push(x.key);
		if (cmphi > 0)
			keySet(x.right, queue, lo, hi);
	}
